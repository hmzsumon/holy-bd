...
const MyOrders = () => {
  ...
  const { orders, loading, error, orderItems } = useSelector(
    ...
    if (status && orderTime) {
      if (+orderTime === dt.getMonth()) {
        const filteredArr = orders.filter(
          (order) =>
            order.status === status &&
            new Date(order.created_at).getMonth() === +orderTime
        );
        ...
        const filteredArr = orders.filter(
          (order) =>
            order.status === status &&
            new Date(order.created_at).getFullYear() === +orderTime
        );
        setFilteredOrders(filteredArr);
      }
    } else if (!status) {
      if (+orderTime === dt.getMonth()) {
        const filteredArr = orders.filter(
          (order) => new Date(order.created_at).getMonth() === +orderTime
        );
        setFilteredOrders(filteredArr);
      } else {
        const filteredArr = orders.filter(
          (order) => new Date(order.created_at).getFullYear() === +orderTime
        );
        setFilteredOrders(filteredArr);
      }
    } else {
      const filteredArr = orders.filter((order) => order.status === status);
      setFilteredOrders(filteredArr);
    }
  ...
  return (
    <>
      <MetaData title='My Orders | Holy Tradres' />
